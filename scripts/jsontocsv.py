    ...
    with open("covid19.csv", "w", newline="", encoding="utf-8") as f_output:
        ...
        for time in range(timstamps):
            ...
            for i in range(count):
                ...
                csv_output.writerow([timestamp, src, dst, weight] + list(dataset["y"][time][region] for region in range(len(dataset["y"][time]))))
            print(count)
load_dataset()
...
from covid19Event import covid19Event
...
path = "./dataset/"
dataset = covid19Event(path, name='englandcovid19')
data = dataset[0]
...
min_dst_idx, max_dst_idx = torch.min(data.dst), torch.max(data.dst)
min_src_idx, max_src_idx = torch.min(data.src), torch.max(data.src)
print(f'min: src {min_src_idx} max: src {max_src_idx}, : min sdt {min_dst_idx.item()} max: dst {max_dst_idx.item()} ')
